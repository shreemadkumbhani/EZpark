...
const connectDB = async () => {
  try {
    await mongoose.connect(
      process.env.MONGO_URI || "mongodb://localhost:27017/parkeasy"
    );
...
const createGeoIndex = async () => {
  try {
    ...
    console.log(
      "📋 Existing indexes:",
      existingIndexes.map((idx) => idx.name).join(", ")
    );
    ...
    const coordIndexResult = await collection.createIndex({
    ...
    console.log(
      "📋 Updated indexes:",
      newIndexes.map((idx) => idx.name).join(", ")
    );
